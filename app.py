...
def order():
    try:
        item = request.form['item']  # Get the item name from the form
        ...
        OrderManager.place_order(db, item, quantity)
...
def admin_add_item():
    ...
    try:
        ...
        if category.lower() == "buffet":
            db.cursor.execute(
                "INSERT INTO buffet_menu (item, section, price) VALUES (%s, %s, %s)",
                (item, section, price),
            )
        elif category.lower() == "a la carte":
            db.cursor.execute(
                "INSERT INTO a_la_carte_menu (item, price) VALUES (%s, %s)",
                (item, price),
            )
        else:
        ...
        db.conn.commit()
        return f"Item '{item}' added successfully.", 200
