...
class DatabaseConnector:
    def __init__(self):  # inistialize the attributes
        self.conn = None
        self.cursor = None
        try:
    ...
    def create_tables(self):             # Entire function OOP concept: Abstraction 
        ...
        for query in queries:
            self.cursor.execute(query)  # OOP concept: Encapsulation(private)
        self.conn.commit()
...
class MenuManager(AbstractMenuManager):
    ...
    def add_buffet_item(self, item, section, price):  # OOP Concept: Encapsulation 
        try:
            query = "INSERT INTO buffet_menu (item, section, price) VALUES (%s, %s, %s)" # query to insert data into the buffet_menu table
            self.db.cursor.execute(query, (item, section, price))  # Refers to the database cursor and Executes the query with values
    ...
    def add_a_la_carte_item(self, item, price): # OOP Concept: Encapsulation 
        try:
            query = "INSERT INTO a_la_carte_menu (item_name, price) VALUES (%s, %s)" # query to insert data into the a_la_carte table
            self.db.cursor.execute(query, (item, price)) # Refers to the database cursor and Executes the query with values
            self.db.conn.commit() # commits the transaction(saves the changes) to the database.
        except Exception as e:
